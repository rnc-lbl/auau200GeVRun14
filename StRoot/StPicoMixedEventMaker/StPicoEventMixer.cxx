...
bool StPicoEventMixer::addPicoEvent(const StPicoDst * picoDst, StHFCuts *mHFCuts)
{
  ...
  for( int iTrk = 0; iTrk < nTracks; iTrk++ ){
    ...
    StMixerTrack *mTrack = new StMixerTrack(pVertex, picoDst->event()->bField(), trk, isTpcPi, isTofPi, isTpcK, isTofK);
...
void StPicoEventMixer::mixEvents(StHFCuts *mHFCuts){
  ...
  for( int iEvt2 = 1; iEvt2 < nEvent; iEvt2++){
	...
	continue;
      StMixerTrack *pion = new StMixerTrack(mEvents.at(iEvt2)->trackAt(iTrk2));
      
      for( int iTrk1 = 0; iTrk1 < kaEvt1.size(); iTrk1++){

	if((mEvents.at(0)->trackAt(kaEvt1.at(iTrk1)).charge() == pion->charge()))
	  continue;
	StMixerTrack *kaon = new StMixerTrack(mEvents.at(0)->trackAt(kaEvt1.at(iTrk1)));

	StMixerPair *pair = new StMixerPair(kaon, pion,
